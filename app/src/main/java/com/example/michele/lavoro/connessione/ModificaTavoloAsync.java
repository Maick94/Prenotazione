...
public class ModificaTavoloAsync extends AsyncTask<String, Void, String> {

    private ProgressDialog loadingDialog;
    private Context context;
    private String tavolo;

    public ModificaTavoloAsync(Context context){
    ...
    protected String doInBackground(String... params) {
        ...
        if(isNetworkAvailable(context)){
            ...
            try{
                ...
                String data = "?tavolo=" + URLEncoder.encode(tavolo, "UTF-8");

                // Link
                String link = "http://databaseandroid.altervista.org/php/Control/ModificaTavoloControl.php" + data;
                Log.d("DEBUG:", link);
    ...
    protected void onPostExecute(String result) {
        ...
        if (result != null) {
            ...
            if(bool) {
                Toast.makeText(context, "Modifica Tavolo avvenuta correttamente!", Toast.LENGTH_SHORT).show();
                Intent intent = new Intent(context, MainActivity.class);
                intent.putExtra("Numero", (Serializable) 0);
                context.startActivity(intent);
            } else
                Toast.makeText(context, "Modifica Tavolo non avvenuta: Errore!", Toast.LENGTH_SHORT).show();
        } else {
